...
const TIMER_SECS = 600;  // 10 min payment window
const PROC_SECS  = 120;  // 2 min fake verification

// ─── MAIN COMPONENT ────────────────────────────────────────────
// Props:
//   isOpen  : boolean
//   onClose : () => void
//   card    : { title: string, price: number, id: string }
export default function PaymentModal({ isOpen, onClose, card }) {
  ...
  useEffect(() => {
    ...
    setPhase("init"); setTxnId(""); setTxnErr(false); setCopied(false);
    setTimer(TIMER_SECS); setProcTimer(PROC_SECS); setInitStep(0); setDots("");

    // 4 fake gateway steps × 650ms each → ~2.6s before QR appears
    let step = 0;
  ...
  // Dots animation while processing
  useEffect(() => {
  ...
  const qrData      = `upi://pay?pa=${UPI_ID}&pn=SecureStore&am=${card.price}&cu=INR&tn=${encodeURIComponent(card.title)}`;
        ...
        <Sheet>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "8px 4px 4px" }}>
            ...
            <p style={{ fontSize: 13, color: "#6b7280", marginBottom: 24, textAlign: "center" }}>
              {card.title} &middot; <span style={{ color: "#4f46e5", fontWeight: 700 }}>₹{card.price}</span>
            </p>
        ...
        <Sheet onClose={onClose}>
          ...
          <div style={{
            ...
            <div>
              ...
              <p style={{ fontSize: 13, fontWeight: 700, color: "#1e1b4b", fontFamily: "monospace", letterSpacing: 0.4 }}>
                {UPI_ID}
              </p>
            </div>
            <motion.button
          ...
          <div style={{ marginBottom: 12 }}>
            <label style={{
              ...
              letterSpacing: 0.5, display: "block", marginBottom: 7,
            ...
            <div style={{
              ...
              boxShadow: txnErr ? "0 0 0 3px #fee2e2" : "none",
              transition: "all 0.2s",
            }}>
              <div style={{ padding: "0 12px", color: "#94a3b8", flexShrink: 0 }}>
                <Hash size={15} />
              </div>
              <input
                value={txnId}
            ...
            <AnimatePresence>
              {txnErr && (
                ...
                >
                  <AlertCircle size={11} /> Please enter the transaction ID from your payment app.
                </motion.p>
              )}
            </AnimatePresence>
          </div>
          ...
          <div style={{
            ...
            <p style={{ fontSize: 11, color: "#92400e", lineHeight: 1.65, margin: 0 }}>
              Pay first in your UPI app, then paste the Transaction ID here. Don't close this window after paying.
            </p>
          </div>
        ...
        <Sheet onClose={onClose}>
          <div style={{ textAlign: "center" }}>
            ...
            <div style={{
              ...
              <span style={{ fontSize: 10, color: "#94a3b8", letterSpacing: 0.5, fontWeight: 600 }}>TXN</span>
            ...
            <p style={{ fontSize: 11, color: "#9ca3af", marginBottom: 22 }}>
              Estimated time: {procMinsLabel} remaining
            </p>
            ...
            <div style={{ textAlign: "left", display: "flex", flexDirection: "column", gap: 12 }}>
              {PROC_STEPS.map((s, i) => (
                ...
                >
                  ...
                  <span style={{
                    fontSize: 13, fontWeight: s.done ? 600 : 400,
                    color: s.done ? "#111827" : "#9ca3af",
                    transition: "all 0.4s",
                  }}>
                    {s.label}
                  </span>
          ...
          >
            ...
            <FadeUp delay={0.25}>
              <div style={{
                ...
                <div style={{
                  ...
                  <span style={{ fontSize: 11, fontWeight: 700, color: "#166534", letterSpacing: 0.4 }}>
                    ACCOUNT CREDENTIALS
                  </span>
                </div>
                {[
                  { label: "Product",  value: card.title },
                  { label: "Email",    value: "user@example.com" },
                  { label: "Password", value: "Acc@2025#secure" },
                  { label: "UID",      value: "8847261920" },
                  ...
                  >
                    ...
                    <span style={{ fontSize: 12, fontWeight: 600, color: "#111827", fontFamily: "monospace" }}>
                      {row.value}
                    </span>
                  </div>
                ))}
            ...
            <FadeUp delay={0.35}>
              <div style={{
                ...
                <AlertCircle size={13} color="#d97706" style={{ flexShrink: 0, marginTop: 1 }} />
                <p style={{ fontSize: 11, color: "#92400e", lineHeight: 1.65, margin: 0 }}>
                  Screenshot these credentials immediately. Contact <strong>@store_admin</strong> on Telegram for support.
                </p>
              </div>
            </FadeUp>
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        borderRadius: 99, margin: "12px auto 18px",
      }} />
